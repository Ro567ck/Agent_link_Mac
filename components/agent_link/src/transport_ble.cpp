...
int L2capEvent(struct ble_l2cap_event* event, void* arg) {
    ...
    case BLE_L2CAP_EVENT_COC_DATA_RECEIVED: {
        ...
        if (!is_img) {
            ...
            // Diagnostic: prove whether the App sends ANY TTS downlink at all. Rate-limited to the first
            // SDU per burst so a real stream doesn't flood the log; silence here = the App never sends.
            static uint32_t s_tts_rx_log_holdoff = 0;
            if (total > 0 && s_tts_rx_log_holdoff == 0) {
                s_tts_rx_log_holdoff = 1;
                ESP_LOGI(TAG, "L2CAP TTS downlink: first SDU received (%u bytes)", total);
            }
