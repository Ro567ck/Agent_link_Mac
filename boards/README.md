...
| `gc2145-camera/`    | ESP32-S3 | GC2145 DVP camera live preview on an ST7789 240x240 LCD; ES8311 push-to-talk mic (hold the key to talk); JPEG snapshot every minute or on the Agent's `camera0` call, with the analysis prompt on the `study_watch` MCP endpoint |
